  ...
  try
    {
      ...
      logger->info("Child of root node are:");                               
                                                                                
      for (OpcUa::Node node : root.GetChildren())                            
        { logger->info("    {}", node); 
          QualifiedName qn = node.GetBrowseName();
          logger->info("Name: {}",qn.Name);
          logger->info("index: {}",qn.NamespaceIndex);
        }
      ...
      //get and browse Server node                                             
      Node server = client.GetServerNode();                                   
      logger->info("Server node is: {}", server);                             
      logger->info("Child of server node are:");                               
                                                                                
      for (OpcUa::Node node : server.GetChildren())                            
        { logger->info("    {}", node);   
          QualifiedName qn = node.GetBrowseName();
          logger->info("Name: {}",qn.Name);
          logger->info("index: {}",qn.NamespaceIndex);
      ...
      logger->info("NsArray node is: {}", nsnode);                       
      ...
      //test different type
      logger->info("set a uint32 number to myvar");
      myvar.SetValue(Variant(8));
