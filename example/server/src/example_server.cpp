...
void RunServer()
{
  ...
  NodeId nid(99, idx);
  NodeId nid1(2020, idx);
  QualifiedName qn("NewObject", idx);
  QualifiedName qn1("NewObject1", idx);
  Node newobject = objects.AddObject(nid, qn);
  Node newobject1 = objects.AddObject(nid1, qn1);
  Node newobject2 = objects.AddObject(idx, "NewObject2");
  logger->info("NewObject node is: {}", newobject);
  logger->info("NewObject1 node is: {}", newobject1);
  logger->info("NewObject2 node is: {}", newobject2);

  //Add a variable and a property with auto-generated nodeid to our custom object
  Node myvar = newobject.AddVariable(idx, "MyVariable", uint16_t(1));
  //Node myvar = newobject.AddObject(idx, "MyVariable");
  Node myprop = newobject.AddVariable(idx, "MyProperty", Variant(8.8));
  Node mymethod = newobject.AddMethod(idx, "MyMethod", MyMethod);

  logger->info("variant(99) type: {}", Variant(99).Type());
  logger->info("variant(9.9) type: {}", Variant(9.9).Type());
  logger->info("variant(true) type: {}", Variant(true).Type());
