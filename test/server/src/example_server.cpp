...
void RunServer()
{
  ...
  uint32_t idx = server.RegisterNamespace("http://test.bs");
  ...
  //browse newobject
  logger->info("Node newobject is: {}", newobject);
  logger->info("Children of newobject are:");                                        
                                                                                 
  for (Node node : newobject.GetChildren())                                          
    {                                                                           
      logger->info("    {}", node);                                             
    } 

  logger->info("Node myvar is: {}", myvar);
  logger->info("Node myprop is: {}", myprop);                                  
  logger->info("Node mymethod is: {}", mymethod);
 
  //browsw root node on server side
  Node root = server.GetRootNode();
  logger->info("Root node is: {}", root);                                       
  logger->info("Children of root are:");
